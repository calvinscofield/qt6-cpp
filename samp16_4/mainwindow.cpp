...
void MainWindow::iniChart()
{
    ...
    axisY->setRange(0, 256); // UInt8采样，数据范围为0~255
    ...
    , ui(new Ui::MainWindow)
{
    ...
    if (device.isNull())
    {
        ...
        ui->groupBoxDevice->setTitle("音频输入设备（无设备）");
    ...
    // calvin注释：原来书本里的代码是ui->spinSampRate->setRange(minSampRate, maxSampRate);有误
    ui->spinSampRate->setRange(minRate, maxRate);

    ui->spinChanCount->setValue(audioFormat.channelCount()); // 通道数
...
void MainWindow::on_comboSampFormat_currentIndexChanged(int index)
{
    ...
    ui->chkBoxShowWave->setEnabled(canShowWave); // 只有采样点格式为UInt8且通道数为1时才能显示曲线
...
{
    // “通道数”SpinBox
    ui->spinBytesPerFrame->setValue(arg1 * (ui->spinBytesPerSamp->value())); // 每帧字节数
...
{
    // “开始采集”按钮
    if (ui->comboSampFormat->currentIndex() == 0) // 检查采样点格式
    {
    ...
    if ((saveToFile) && (fileName.isEmpty())) // 检查数据记录文件
    {
    ...
    myDevice->openDAQ(m_bufferSize, showWave, lineSeries, saveToFile, fileName); // 打开I/O设备
