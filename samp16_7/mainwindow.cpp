...
void MainWindow::showCameraDeviceInfo(QCameraDevice *device)
{
    ...
    // 拍照时的图片分辨率
    ui->comboCam_PhotoRes->clear(); // 支持的图片分辨率
    ...
    foreach (QSize size, device->photoResolutions())
    {
        ...
        ui->comboCam_PhotoRes->addItem(str); // 支持的图片分辨率
        ui->comboImage_Resolution->addItem(str, size); // 拍照使用的分辨率，添加用户数据
    }

    // 视频分辨率
    ui->comboCam_VideoRes->clear(); // 支持的视频分辨率
    ui->comboCam_FrameRate->clear(); // 支持的帧率范围
    ui->comboVideo_Resolution->clear(); // 录像使用的分辨率
    foreach (QCameraFormat format, device->videoFormats())
    {
        QSize size = format.resolution();
        QString str = QString::asprintf("%d X %d", size.width(), size.height());
        ui->comboCam_VideoRes->addItem(str); // 支持的视频分辨率
        ui->comboVideo_Resolution->addItem(str, size); // 录像用的分辨率，添加用户数据
        str = QString::asprintf("%.1f ~ %.1f", format.minFrameRate(), format.maxFrameRate());
        ui->comboCam_FrameRate->addItem(str); // 帧率范围
    }
}
...
void MainWindow::showCameraSupportFeatures(QCamera *aCamera)
{
    ...
    ui->chkBoxCam_Exposure->setCheckable(supported);
    ...
    , ui(new Ui::MainWindow)
{
    ...
    for (int i = 0; i < QMediaDevices::videoInputs().size(); i++)
    {
        ...
        {
            ui->comboCam_List->addItem(device.description() + "(默认)", QVariant::fromValue(device)); // 添加QCameraDevice类型的用户数据
            index = i;
        }
        else
    ...
    camera->setCameraDevice(defaultCameraDevice); // 设置使用默认视像头设备
    ...
    imageCapture->setQuality(QImageCapture::VeryHighQuality); // 抓图编码品质
    ...
    QString filename = ":/sound/image/shutter.wav"; // 资源文件中的文件
    ...
    // 3. 视频编码和文件格式，添加到“录像设置”分组框的下拉列表框里，用于选择
    QMediaFormat format;
    foreach (QMediaFormat::VideoCodec codec, format.supportedVideoCodecs(QMediaFormat::Encode)) // 支持的视频编码
        ui->comboVideo_Codec->addItem(QMediaFormat::videoCodecDescription(codec), QVariant::fromValue(codec));
    foreach (QMediaFormat::FileFormat fileFormat, format.supportedFileFormats(QMediaFormat::Encode)) // 支持的文件格式
        ui->comboVideo_FileFormat->addItem(QMediaFormat::fileFormatDescription(fileFormat), QVariant::fromValue(fileFormat));
}
...
{
    QVariant var = ui->comboCam_List->itemData(index); // 获取用户数据
    QCameraDevice device = var.value<QCameraDevice>(); // 用户数据转换为QCameraDevice类型
...
{
    // 摄像头状态变化
    ui->actStartCamera->setEnabled(!active); // “开启摄像头”按钮
    ui->actStopCamera->setEnabled(active); // “关闭摄像头”按钮
    ui->actVideoRecord->setEnabled(active); // “开始录像”按钮
    ui->comboCam_List->setEnabled(!active); // “摄像头”下拉列表
    m_isWorking = active;
}
...
void MainWindow::do_image_captured(int id, const QImage &preview)
{
    ...
    ui->labImage->setPixmap(QPixmap::fromImage(scaledImage)); // 显示拍摄的图片
...
void MainWindow::closeEvent(QCloseEvent *event)
{
    if (m_isWorking)
    {
        if (recorder->recorderState() == QMediaRecorder::RecordingState) // 正在录像
            recorder->stop(); // 停止录像
        camera->stop(); // 关闭摄像头
    }
    event->accept();
...
void MainWindow::on_actVideoRecord_triggered()
{
    ...
    recorder->setQuality(QMediaRecorder::Quality(index)); // 设置编码品质
